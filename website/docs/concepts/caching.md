...
Configure one or more remote caches in `.hephconfig`:
...
order; the first cache holding a matching entry is a cache hit. Confirming the
hit only costs a small metadata fetch — the output bytes themselves transfer
later and lazily, and only for the outputs something actually reads. Latency
is measured once per process (or loaded from a persisted file) and refreshed
